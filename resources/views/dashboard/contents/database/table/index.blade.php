    ...
    <div class="content container-fluid">
        <div class="row">
            ...
            <div class="col-xs-8 text-right m-b-20">
                <a href="#" class="btn btn-info rounded pull-right" data-toggle="modal" data-target="#import"><i
                        class="fa fa-cloud-upload"></i> Import Table</a>
                <a href="#" class="btn btn-primary rounded pull-right" data-toggle="modal" data-target="#create"><i
                        class="fa fa-plus"></i> Create Table</a>
        ...
        <div class="row">
            <div class="col-md-12">
                <div class="table-responsive">
                    <table
                        class="table custom-table {{ config('dashboard.modules.table.use_datatable') ? 'datatable' : null }}">
                        <thead>
                            <tr>
                        ...
                        <tbody>
                            @if ($database->user_table)
                                <tr>
                                    <td style="color:#ccc">0</td>
                                    <td colspan="5"><i class="fa fa-table"></i> users</td>
                                </tr>
                            @endif
                            @if ($database->user_permission)
                                <tr>
                                    <td style="color:#ccc">0</td>
                                    <td colspan="5"><i class="fa fa-table"></i> permissions</td>
                                </tr>
                                <tr>
                                    <td style="color:#ccc">0</td>
                                    <td colspan="5"><i class="fa fa-table"></i> roles</td>
                                </tr>
                                <tr>
                                    <td style="color:#ccc">0</td>
                                    <td colspan="5"><i class="fa fa-table"></i> role_has_permissions</td>
                                </tr>
                                <tr>
                                    <td style="color:#ccc">0</td>
                                    <td colspan="5"><i class="fa fa-table"></i> model_has_permissions</td>
                                </tr>
                                <tr>
                                    <td style="color:#ccc">0</td>
                                    <td colspan="5"><i class="fa fa-table"></i> model_has_roles</td>
                                </tr>
                            @endif
                            @foreach ($table_list as $item)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('database.table.column.index', [$database, $item]) }}"><i
                                                class="fa fa-table"></i> {{ $item->name }}</a>
                                    </td>
                                    <td>
                                        <span style="color: #666">
                                            {{$item->pivot ? 'PIVOT': null}}
                                        </span>
                                    </td>
                                    <td style="width:100px">
                                        <div class="dropdown action-label">
                                            <a class="btn btn-white btn-xs rounded dropdown-toggle" data-toggle="dropdown"
                                                aria-expanded="false"><i class="fa fa-dot-circle-o text-primary"></i> Move
                                                <i class="caret"></i></a>
                                            <ul class="dropdown-menu">
                                                <li><a href="@if (!$loop->first) {{ route('move_item', ['model' => 'Table', 'id' => $item->id, 'value' => 'top']) }} @endif"><i class="fa fa-dot-circle-o text-success"></i>
                                                        Move to first</a></li>
                                                <li><a href="@if (!$loop->first) {{ route('move_item', ['model' => 'Table', 'id' => $item->id, 'value' => 'up']) }} @endif"><i class="fa fa-arrow-up text-success"></i> Move
                                                        up</a></li>
                                                <li><a href="@if (!$loop->last) {{ route('move_item', ['model' => 'Table', 'id' => $item->id, 'value' => 'down']) }} @endif"><i class="fa fa-arrow-down text-danger"></i> Move
                                                        down</a></li>
                                                <li><a href="@if (!$loop->last) {{ route('move_item', ['model' => 'Table', 'id' => $item->id, 'value' => 'bottom']) }} @endif"><i class="fa fa-dot-circle-o text-danger"></i>
                                                        Move to last</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                        ...
                                        <div class="dropdown action-label">
                                            <a class="btn btn-white btn-sm rounded dropdown-toggle" data-toggle="dropdown"
                                                aria-expanded="false"><i
                                                    class="fa fa-dot-circle-o {{ $item->active ? 'text-success' : 'text-danger' }}"></i>
                                                {{ $item->active ? 'Active' : 'Inactive' }} <i class="caret"></i></a>
                                            <ul class="dropdown-menu">
                                                <li><a href="@if (!$item->active) {{ route('change_activation', ['model' => 'Table', 'id' => $item->id, 'active' => true]) }} @endif"><i class="fa fa-dot-circle-o text-success"></i>
                                                        Active</a></li>
                                                <li><a href="@if ($item->active) {{ route('change_activation', ['model' => 'Table', 'id' => $item->id, 'active' => false]) }} @endif"><i class="fa fa-dot-circle-o text-danger"></i>
                                                        Inactive</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                    <td style="width:10px">
                                        <div class="dropdown">
                                            ...
                                            <ul class="dropdown-menu pull-right">
                                                <li><a href="{{ route('database.table.api_crud', [$database, $item]) }}"><i
                                                            class="fa fa-eye m-r-5"></i> Make API CRUD</a></li>
                                                <li><a href="#" data-toggle="modal" data-target="#delete"
                                                        ng-click="delete_url = '{{ route('database.table.destroy', [$database, $item]) }}'"><i
            ...
            <div class="modal-content modal-lg">
                ...
                <div class="modal-body">
                    <form action="{{ route('database.table.store', $database) }}" method="POST" autocomplete="off">
                        ...
                        {!! BootForm::hidden('pivot', 0, false) !!}
                        {!! BootForm::checkbox('pivot', 'Pivot Table', 1, old('pivot') ?? false, ['ng-model' => 'pivot']) !!}
                    ...
                    <div class="card-box">
                        ...
                        <div class="tab-content" style="min-height: 500px">
                            <div class="tab-pane active" id="bottom-justified-tab1">
                                <form action="{{ route('database.import', $database) }}" method="POST" autocomplete="off"
                                    ...
                                    <label style="
                                        ...
                                        <input type="file" id="files" name="files[]" multiple
                                            onchange="javascript:updateList()" style="display: none" />
                                        <i class="fa fa-cloud-upload"></i> Choose Files
                                    </label>
                                    <ul class="list-group" id="fileList"></ul>
                                    <div class="m-t-20 text-center">
                                        <button class="btn btn-primary btn-lg"><span class="fa fa-upload"></span>
                                            Import</button>
                                    </div>
                                </form>
    ...
    <script>
        ...
        updateList = function() {
            ...
            for (var i = 0; i < input.files.length; ++i) {
                children += '<li class="list-group-item"><span class="fa fa-table"></span> ' + input.files.item(i)
                    .name +
                    '<span style="margin-left:15px" class="fa fa-times text-danger pull-right" onclick="remove(this,' +
                    i + ')"></span>' + '<span class="text-muted pull-right">' + input.files.item(i).size / 100 +
                    'KB</span>' + '</li>'
            }
