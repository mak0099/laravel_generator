...
<div class="content container-fluid">
    <div class="row">
        ...
        <div class="col-xs-8 text-right m-b-20">
            <a href="#" class="btn btn-primary rounded pull-right" data-toggle="modal" data-target="#create"><i class="fa fa-plus"></i> Create News</a>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="table-responsive">
                <table class="table custom-table {{ config('dashboard.modules.news.use_datatable') ? 'datatable' : null }}">
                    <thead>
                        <tr>
                            ...
                            <td>
                                <div class="product-det">
                                    ...
                                    <div class="product-desc">
                                        <h2><a href="{{ route('news.show', $item) }}">{{ str_limit($item->title,60) }}</a> <span>{{ str_limit($item->description,70) }} </span></h2></div>
                                </div>
                            </td>
                            <td>
                                <div class="dropdown action-label">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li><a href="@if(!$item->featured){{ route('change_feature',['model'=>'News', 'id'=>$item->id, 'featured'=>true]) }}@endif"><i class="fa fa-dot-circle-o text-success"></i> Featured</a></li>
                                        <li><a href="@if($item->featured){{ route('change_feature',['model'=>'News', 'id'=>$item->id, 'featured'=>false]) }}@endif"><i class="fa fa-dot-circle-o text-primary"></i> Normal</a></li>
                                    </ul>
                                </div>
                            </td>
                            <td>
                                <div class="dropdown action-label">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li><a href="@if(!$item->active){{ route('change_activation',['model'=>'News', 'id'=>$item->id, 'active'=>true]) }}@endif"><i class="fa fa-dot-circle-o text-success"></i> Active</a></li>
                                        <li><a href="@if($item->active){{ route('change_activation',['model'=>'News', 'id'=>$item->id, 'active'=>false]) }}@endif"><i class="fa fa-dot-circle-o text-danger"></i> Inactive</a></li>
                                    </ul>
                                </div>
                            </td>
                            <td class="text-right">
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu pull-right">
                                        <li><a href="{{ route('news.show', $item) }}"><i class="fa fa-eye m-r-5"></i> View Detail</a></li>
                                        <li><a href="{{ route('news.edit', $item) }}"><i class="fa fa-pencil m-r-5"></i> Edit</a></li>
                                        <li><a href="#" data-toggle="modal" data-target="#delete" ng-click="delete_url = '{{ route('news.destroy', $item) }}'"><i class="fa fa-trash-o m-r-5"></i> Delete</a></li>
                                    </ul>
                                </div>
                            </td>
                        </tr>
            ...
            <div class="modal-body">
                <form action="{{ route('news.store') }}" method="POST" autocomplete="off" enctype="multipart/form-data">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-9">
                                    <label class="radio-inline">
                                        @php
                                            $featured_remain_count = \App\Models\News::featured_remain_count();
                                        @endphp
                    ...
                    <div class="m-t-20 text-center">
                        <button class="btn btn-primary btn-lg">Create News</button>
                    </div>
                </form>
            </div>
        </div>
